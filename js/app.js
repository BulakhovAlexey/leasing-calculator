(() => {
    ...
    (() => {
        ...
        function updatePercentNum(priceAvto, fee) {
            ...
            percentNum.innerText = Math.round(removeSpaces(fee.value) / removeSpaces(priceAvto.value) * 100) + "%";
        }
        function updateRangeFromFee(priceAvto) {
            return .1 * removeSpaces(priceAvto.value);
        }
        function updateRangeToFee(priceAvto) {
            return .6 * removeSpaces(priceAvto.value);
        }
        function updateFinalValues() {
            const totalSum = document.getElementById("total-sum");
            const feePerMounth = document.getElementById("fee-per-mounth");
            feePerMounth.innerText = addSpaces(Math.round((removeSpaces(priceOfAvtoValue.value) - removeSpaces(initialFeeValue.value)) * (.05 * Math.pow(1.05, leasingTermValue.value) / (Math.pow(1.05, leasingTermValue.value) - 1))));
            totalSum.innerText = addSpaces(Math.round(+removeSpaces(initialFeeValue.value) + leasingTermValue.value * +removeSpaces(feePerMounth.innerText)));
        }
        ...
        if (priceOfAvtoSlider) nouislider.create(priceOfAvtoSlider, {
        ...
        if (initialFeeSlider) nouislider.create(initialFeeSlider, {
        ...
        if (leasingTermSlider) nouislider.create(leasingTermSlider, {
        ...
        const app_elements = document.querySelectorAll("._col-upper");
        app_elements.forEach((el => {
        ...
        calculatorForm.addEventListener("submit", (e => {
            ...
            setTimeout((() => {
                ...
                const finalSum = document.getElementById("total-sum").innerText.replace(/ /g, "");
                const feePerMounth = document.getElementById("fee-per-mounth").innerText.replace(/ /g, "");
                const data = {
                    avtoPrice,
                    initialFee,
                    leasingTerm,
